...
class LabTaskNewton extends Component {
    constructor() {
        super()
        this.state = {
            data: [],
    ...
    calculateDot = (e) => {
        e.preventDefault();
            ...
            let query = this.state.type === 'equation' ? queryTask4 : queryTask4Differ
            findSolutionByNewton({min: +this.state.min, max: +this.state.max}, +this.state.newton, query)
                .then((result) => {
                    if (result === null) {
